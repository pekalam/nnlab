...
namespace NNLab.Tests
{
    ...
    public class NavigationBreadcrumbsTests
    {
        ...
        public void Save_and_restore_breadcrumbs_test()
        {
            ...
            //save
            vm.SaveBreadcrumbsForModule(DataModule.NavIdentifier);

            //assert that breadcrumbs are saved and cleared
            vm.PreviousBreadcrumbs[DataModule.NavIdentifier].Select(b => b.ViewName).Should().BeEquivalentTo("view1", "view11");
            ...
            //save
            vm.SaveBreadcrumbsForModule(NeuralNetworkModule.NavIdentifier);

            //assert that breadcrumbs are saved and cleared
            vm.PreviousBreadcrumbs[NeuralNetworkModule.NavIdentifier].Select(b => b.ViewName).Should().BeEquivalentTo("view2");
            vm.Breadcrumbs.Count.Should().Be(0);

            //restore for data module
            vm.TryRestoreBreadrumbsForModule(DataModule.NavIdentifier);
            //assert
